...
def test_failed_auth_is_audited(api_client, capture_audit):
    resp = api_client.get("/campaigns", headers={"Authorization": "Bearer wrong-token"})
...
def test_session_lifecycle_is_audited(api_client, capture_audit):
    cid = api_client.post("/campaigns", headers=AUTH).json()["campaign_id"]

    api_client.post(f"/campaigns/{cid}/session", headers=AUTH)
    api_client.post(f"/campaigns/{cid}/session/takeover", headers=AUTH, json={})
    api_client.request(
        "DELETE",
    ...
    # The dev campaign_id is opaque; no token value is logged.
