# ADR-023: Session lease — single active session per campaign

**Status**: Accepted | **Date**: 2026-06-28 | **Branch**: `feat/004-auth-obs`

> Renumbered from slice 004's ADR-018 by spec 006 (ADR-020 numbering policy) — the original numbers collided with ADRs 017-019 on `dev`.
